...
class TestArithmetic():
    ...
    def arithmetic_mesh_vector_setup(self):
        ...
        fracs_vector_tag = self.mesh_1_vector.mesh.tag_get_handle("cell_fracs", 
                                                          2,
                                                          types.MB_TYPE_DOUBLE,
                                                          types.MB_TAG_DENSE,
                                                          create_if_missing=True)

        fracs_vector_data = [[1.0, 2.0], [1.5, 2.5], [-1.0, -1.5], [-2.0, -2.5]]        
        self.mesh_1_vector.mesh.tag_set_data(fracs_vector_tag, self.volumes1, fracs_vector_data)
        
        self.mesh_2_vector = Mesh(structured_coords=[[-1, 0, 1], [-1, 0, 1], [0, 1]],
                                  structured = True)
        volumes2 = list(self.mesh_2_vector.structured_iterate_hex("xyz"))
        fracs_vector_tag = self.mesh_2_vector.mesh.tag_get_handle("cell_fracs",
                                                          2,
                                                          types.MB_TYPE_DOUBLE,
                                                          types.MB_TAG_DENSE,
                                                          create_if_missing=True)

        fracs_vector_data = [[15.0, 25.0], [10.0, 20.0], [-15.0, -25.0], [-20.0, -25.0]]
        self.mesh_2_vector.mesh.tag_set_data(fracs_vector_tag, volumes2, fracs_vector_data)
    ...
    def test_add_vectors_mesh(self):
        ...
        self.mesh_1_vector._do_op(self.mesh_2_vector, "cell_fracs", "+")
        exp_res = [[16, 27], [11.5, 22.5], [-16, -26.5], [-22, -27.5]]
        fracs_tag = self.mesh_1_vector.mesh.tag_get_handle("cell_fracs")
        obs_res = self.mesh_1_vector.mesh.tag_get_data(fracs_tag, self.volumes1) 
