...
def test_rtflux_3D():
    ...
    assert np.allclose(
        flux[0], [2.66320088e-06, 7.48262958e-05, 1.20151503e-04, 2.34609601e-05]
    )
...
def test_rtflux_2D():
    ...
    # test energy ordering
    assert np.allclose(
        flux[0], [1.66764798e-03, 4.59426961e-02, 7.35252284e-02, 1.54202809e-02]
    )
    # test spatial ordering
    exp = [
        1.54202809e-02,
...
def test_rt_flux_1D():
    ...
    flux = m.tag[:]
    exp = [
        [1.13102481e-03, 2.48423595e-02, 4.07499865e-02, 1.12382315e-02],
