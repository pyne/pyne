...
def test_xs_cache_sigma_f_n():
    ...
    assert id(sigma_f_n_U235) != id(from_cache)
    assert id(from_cache) == id(xs_cache[922350, "fiss"])
...
def test_xs_cache_sigma_a_n():
    ...
    assert id(sigma_a_n_H1) != id(from_cache)
    assert id(from_cache) == id(xs_cache[10010, "abs"])
...
def test_xs_cache_set_E_g():
    ...
    assert E_g.shape == (2,)
    assert id(E_g) == id(xs_cache["E_g"])

    # Assert that the cache has been reloaded
    xs_cache["E_g"] = [10.0, 2.0, 1.0]
    assert id(E_g) != id(xs_cache["E_g"])
    assert len(E_g) == 2
    assert len(xs_cache["E_g"]) == 3

    # Test auto-clearing
    xs_cache[10010, "fiss"]
    assert (10010, "fiss") in xs_cache
    xs_cache["E_g"] = [10.0, 8.0, 2.0, 1.0]
    assert xs_cache["phi_g"] is None
    assert not (10010, "fiss") in xs_cache


def text_xs_get_reaction():
    xs_cache.clear()
    pytest.raises(KeyError, xs_cache[10010, 1089, 300])
