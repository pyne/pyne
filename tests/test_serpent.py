...
def test_parse_res1():
    ...
    rank0 = res["IDX"]
    assert res["idx"] + 1 == rank0
    for key in res:
        if isinstance(res[key], np.ndarray):
            assert res[key].shape[0] == rank0
...
def test_parse_dep2():
    ...
    for key in dep:
        if key.endswith("_VOLUME"):
            ...
            assert len(dep[key]) == shape[0]
            assert len(dep[key][0]) == shape[1]

    # Check values
    assert_array_equal(dep["BU"], [0.00000e00, 1.00000e-01, 1.00000e00])
    assert dep["i621510"] == 22
...
def test_parse_det1():
    ...
    for key in det:
        if "_" in key:
            assert isinstance(det[key], int)
        elif isinstance(det[key], np.ndarray):
            assert det[key].shape[1] in [3, 13]
