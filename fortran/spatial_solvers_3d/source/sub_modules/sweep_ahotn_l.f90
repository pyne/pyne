...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            b(1)=c*e_ahotn_l(1,i,j,k) + s(0,0,0,i,j,k,g)/sig
            b(2)=c*e_ahotn_l(2,i,j,k) + s(0,0,1,i,j,k,g)/sig
            b(3)=c*e_ahotn_l(3,i,j,k) + s(0,1,0,i,j,k,g)/sig
            b(4)=c*e_ahotn_l(4,i,j,k) + s(1,0,0,i,j,k,g)/sig

            ! call AHOTN kernel
						IF((solver == "AHOTN") .and. (solvertype == "LN")) THEN
	            call  ln_kernel(x,y,z,mu,eta,xi,incx,incy,incz,sig,c,fx,fy(:,i,nfy),fz(:,i,j,nfy,nfz),b)
						ELSE IF((solver == "AHOTN") .and. (solvertype == "LL")) THEN
		          call  ll_kernel(x,y,z,mu,eta,xi,incx,incy,incz,sig,c,fx,fy(:,i,nfy),fz(:,i,j,nfy,nfz),b)
						END IF
            ! Update the scalar flux solution
            f_ahotn_l(1,i,j,k,g) = f_ahotn_l(1,i,j,k,g) + w(n)*b(1)
            f_ahotn_l(2,i,j,k,g) = f_ahotn_l(2,i,j,k,g) + w(n)*b(2)
            f_ahotn_l(3,i,j,k,g) = f_ahotn_l(3,i,j,k,g) + w(n)*b(3)
            f_ahotn_l(4,i,j,k,g) = f_ahotn_l(4,i,j,k,g) + w(n)*b(4)
