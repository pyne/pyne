...
"""    
Mg = Material({120240000: 0.7899, 120250000: 0.10000000000000002, 120260000: 0.11010000000000002})
...
test Mg
"""
def test_material1(Mg, output_Material_library):
    for material in output_Material_library.iteritems():
        if material[0] == 'Mg' :
            assert_almost_equal(material[1].comp, Mg.comp, places=4)
...
def test_material2(Nitrogen, output_Material_library):
    for material in output_Material_library.iteritems():
        if material[0] == 'Nitrogen' :
            assert_almost_equal(material[1].comp, Nitrogen.comp, places=4)

"""
#test Steel, Stainless 321
"""
def test_material3(Steel, output_Material_library):
    for material in output_Material_library.iteritems():
        if material[0] == 'Steel, Stainless 321' :
...
def main():
    ...
    output_lib=load_output('output_6.h5m')
    #test materials
    test_material1(Mg,output_lib)
    test_material2(Nitrogen,output_lib)
    test_material3(Steel,output_lib)
