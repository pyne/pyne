...
class Njoy99(object):
    ...
    def pendf_run(self, eaf=0):
        """Run the pointwise PENDF file from the ENDF raw data using the
        MODER, RECONR, BROADR, PURR (if dilutions present), and THERMR
        modules. This is the starting point for all other datatype generations
        including DRAGLIB, ACE, WIMSD, etc.
        ...
        # At this point, you would need the tape20, tape29 files
    ...
    def gendf_run(self, eaf=0):
        """Run a multigroup GENDF file using the MODER and GROUPR
        modules. This requires that the pendf() method has already been called.
        ...
        # TODO check that pendf has been called
    ...
    def gamma_run(self):
        """Run NJOY99 with the photo-atomic (gamma) group ENDF file using the
        MODER, RECONR, and GAMINR modules.
        """
    ...
    def draglib_input(self, fp=0):
        ...
        if self.oldlib:
            os.system("mv ../" + self.oldlib +
                      " draglib" + evaluation_name_base)
        os.chdir(self.evaluation_name)
        ...
        if os.path.isfile("draglib" + evaluation_name_base):
            ...
            if not os.path.isfile(name1):
                os.system("cp draglib" + evaluation_name_base +
                          " draglib" + evaluation_name_base + ".bis")
                os.system("gzip draglib" + evaluation_name_base + ".bis")
            else:
                name2 = "draglib" + evaluation_name_base + ".bis2.gz"
                os.system("cp draglib" + evaluation_name_base +
                          " draglib" + evaluation_name_base + ".bis2")
            ...
            print(" append data for " + self.hmat +
                  " to existing draglib file")
        else:
            iold = 0
        ...
        if self.dilutions:
            ...
            text_data = ("moder\n20 -21\nmoder\n24 -25\ndragr\n-21 0 -25 0 0 "
                         "%(iold)d 30 %(fp)d/\n" % self.__dict__)
            if iold == 0:
                text_data += ("'draglib from %(evaluation_name)s"
                              " at %(htime)s'/\n" % self.__dict__)
            text_data += ("%(mat)d %(hmat)s /\n'%(hmat)s from "
                          "%(evaluation_name)s (%(mat)d) at %(htime)s' /\n"
                          "0.1 1.0E10 /\n0/\nstop\n" % self.__dict__)
    ...
    def draglib_run(self, fp=0):
        """Run a DRAGLIB file using the MODER and DRAGR modules and
        add/update the new isotopic data in the DRAGLIB file.
    ...
    def burnup(self):
        ...
        if chain_file_name not in list_files:
            print("Make the burnup chain"
                  " file named {0}".format(chain_file_name))
            data_dict = {}
            mat_dict = {}
            ...
            print("Use existing burnup chain "
                  "file named {0}".format(chain_file_name))
