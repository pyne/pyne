...
def test_map_str_uint():
    ...
    uismap = isinstance(4043370667L, Mapping) 
    m['Aha'] = 42L
    m['Take'] = 4043370667L
    import pprint
    pprint.pprint(m)
    ...
    if uismap:
        for key, value in m['Take'].items():
            print key, value, 4043370667L[key]
            if isinstance(value, np.ndarray):
                assert_almost_equal(value, 4043370667L[key])
            else:
                assert_equal(value, 4043370667L[key])
    else:
        assert_almost_equal(m['Take'], 4043370667L)
...
def test_map_uint_str():
    ...
    m = stlcontainers.MapUIntStr({4043370667L: 'Take', 42L: 'Aha'})
    assert_equal(len(m), 2)
    if uismap:
        for key, value in m[4043370667L].items():
            if isinstance(value, np.ndarray):
                print key, value, 'Take'[key]
                assert_equal(value, 'Take'[key])
            else:
...
def test_map_uint_uint():
    ...
    if uismap:
        ...
        assert_almost_equal(m[65], 4043370667L)

    m = stlcontainers.MapUIntUInt({4043370667L: 65, 42L: 1})
    assert_equal(len(m), 2)
    if uismap:
        for key, value in m[4043370667L].items():
            if isinstance(value, np.ndarray):
        ...
        assert_almost_equal(m[4043370667L], 65)
...
def test_map_uint_double():
    ...
    m = stlcontainers.MapUIntDouble({4043370667L: 42.42, 42L: 1.0})
    assert_equal(len(m), 2)
    if uismap:
        for key, value in m[4043370667L].items():
            if isinstance(value, np.ndarray):
        ...
        assert_almost_equal(m[4043370667L], 42.42)
