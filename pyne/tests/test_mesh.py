...
class MeshTest(unittest.TestCase):
    ...
    def test_create_by_set(self):
        a =  self.mesh.createStructuredMesh( [0,0,0,1,1,1], 
                                             i=[1,2], j=[1,2], k=[1,2], 
                                             create_set=True )
        sm = Mesh(mesh = self.mesh, str_set=a, structured=True)
    ...
    def test_hex_volume(self):
        ...
        self.assertEqual(sm.str_get_hex_volume(1,1,0), 4)
        self.assertEqual(sm.str_get_hex_volume(1,1,1), 8)
    ...
    def test_str_get_vertex(self):
        ...
        sm = Mesh(str_coords=[x_range, y_range, z_range], structured=True)

        for i,x in enumerate(x_range):
            for j,y in enumerate(y_range):
                for k,z in enumerate(z_range):
                    print i, j, k
                    vtx = sm.str_get_vertex(i,j,k)
...
class StrMeshIterateTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sm = Mesh(structured=True,\
                       str_coords =[range(10,15), range(21,25), range(31,34)])   
        self.I = range(0,4)
        self.J = range(0,3)
        self.K = range(0,2)
    ...
    def test_iterate_3d(self):        
        ...
        it = self.sm.str_set.iterate( iBase.Type.region, 
        ...
        for it_x, sm_x in izip( it, self.sm.str_iterate_hex() ):
...
class StrPerfTest(unittest.TestCase):
    ...
    def test_large_iterator(self):
        ...
        big = Mesh(str_coords=[range(1,100), range(101,200), range(201,300)], 
                   structured = True)
        print "iterating (1)"
