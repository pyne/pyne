...
from pyne.rx_data import double_spin_dict
import nose 
...
def test_unresolved_resonances_a():
    ...
    assert(np.array_equal(new_exp_D, new_obs_D))
    assert(np.array_equal(new_exp_GNO, new_obs_GNO))
...
def test_unresolved_resonances_b():
    ...
    assert(np.array_equal(exp_419_GF, obs_419_GF))
...
def test_double_spin_dict():
    subject = double_spin_dict({(3.499999999998, 2, 1):{'A':'a', 'B':'b'},
