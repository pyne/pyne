...
SINGLE_EDIT_DESCRIPTORS = ['A', 'B', 'D', 'E', 'F', 'G', 'I', 'L', 'O', 'P',
                           'S', 'T', 'X', 'Z', ':', '/']
H_EDIT_DESCRIPTOR = ['H']
LEFT_PARENS = ['(']
RIGHT_PARENS = [')']
    ...
    while True:
        ...
        # Read in a quoted string
            ...
            while True:
                ...
                if (c0 == delim) and (c1 == delim):
                    ...
                    raise InvalidFormat('Premature end of quoted string in '
                                        'format')
                else:
                    buff = buff + c0
            tokens.append(Token('QUOTED_STRING', buff))
        # Read in an integer
            ...
            if (c0 in SIGNS) and (c1 not in DIGITS):
                ...
                raise InvalidFormat("Orphaned sign '{0:s}' with no digits "
                                    "at position {1:d}".format(c0, s))
            buff = c0
            while True:
                s = s + 1
                c0, c1, c2 = _get_chars(format, s)
        ...
        # Read in an H edit descriptor
            ...
            if len(tokens) > 0 and tokens[-1].type in ('NZUINT', 'UINT'):
                ...
                raise InvalidFormat("Missing H descriptor number argument at "
                                    "position {0:d}".format(s))
        ...
        # Read in right parens
            ...
            raise InvalidFormat('Character {0:s} not recognised at '
                                'position {1:d}'.format(c0, s))
    return tokens
