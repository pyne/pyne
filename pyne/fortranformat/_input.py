...
def input(eds, reversion_eds, records, num_vals=None):
    ...
    if (num_vals > num_out_eds) and (num_rev_out_eds == 0):
        raise ValueError('Not enough output edit descriptors in reversion '
                         'format to output {0:d} values'.format(num_vals))
    ...
    while True:
        ...
        if isinstance(ed, QuotedString):
            raise InvalidFormat('Cannot have string literal in an input '
                                'format')
            ...
            state['position'] = \
                min(state['position'] + ed.num_chars, len(record))
            ...
            # Difficult to know what wanted since do not know type of input
            # variable
            ...
            while not resolved:
                ...
                if ed_name.upper() in ('F', 'E', 'D', 'EN', 'ES'):
                    ...
                    try:
                        val, state = read_integer(trial_ed,
                                                  state.copy(),
                                                  record)
                        vals.append(val)
                        resolved = True
                    ...
                    trial_ed.width = ed.width
                    try:
                        val, state = read_logical(trial_ed,
                                                  state.copy(),
                                                  record)
                        vals.append(val)
                        resolved = True
                    ...
                    trial_ed.width = ed.width
                    try:
                        val, state = read_string(trial_ed,
                                                 state.copy(),
                                                 record)
                        vals.append(val)
                        resolved = True
                    ...
                    raise ValueError('G edit descriptor not permitted in '
                                     'config.G_INPUT_TRIAL_EDS')
                else:
                    raise ValueError('Unrecognised trial edit descriptor '
                                     'string in config.G_INPUT_TRIAL_EDS')
...
# pylint: enable=bad-indentation
    ...
    try:
        ...
        if state['exception_on_fail']:
            raise ValueError('{0:s} is not a valid input for one of integer, '
                             'octal, hex or binary'.format(substr))
        else:
            return (None, state)
    return (val, state)
...
def read_logical(ed, state, record):
    ...
    if len(teststr):
        ...
        raise ValueError('{0:s} is not a valid boolean input'.format(substr))
    if teststr == 'T':
        val = True
    elif teststr == 'F':
        val = False
        ...
        if state['exception_on_fail']:
            raise ValueError('{0:s} is not a valid boolean input'
                             .format(substr))
        else:
            val = None
    return (val, state)
...
def read_float(ed, state, record):
    ...
    try:
        ...
        if state['exception_on_fail']:
            raise ValueError('{0:s} is not a valid input as for an E, ES, EN '
                             'or D edit descriptor'.format(substr))
        else:
            return (None, state)
