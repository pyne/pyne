...
def ext_air_df(nuc, source=0, get_errors=False):
    ...
    if isinstance(nuc, int):
        ext_air = cpp_data.ext_air_df(<int> nuc, <int> source, get_errors)
        ratio = cpp_data.ratio(<int> nuc, <int> source, get_errors)
    elif isinstance(nuc, basestring):
        ext_air = cpp_data.ext_air_df(<char *> nuc, <int> source, get_errors)
        ratio = cpp_data.ratio(<char *> nuc, <int> source, get_errors)
    else:
        raise pyne.nucname.NucTypeError(nuc)

    return ext_air, ratio
...
def ext_soil_df(nuc, source=0, get_errors=False):
    ...
    if isinstance(nuc, int):
        ext_soil = cpp_data.ext_soil_df(<int> nuc, <int> source, get_errors)
    elif isinstance(nuc, basestring):
        ext_soil = cpp_data.ext_soil_df(<char *> nuc, <int> source, get_errors)
    else:
        raise pyne.nucname.NucTypeError(nuc)
...
def ingest_df(nuc, source=0, get_errors=False):
    ...
    if isinstance(nuc, int):
        ingest = cpp_data.ingest_df(<int> nuc, <int> source, get_errors)
        fluid_frac = cpp_data.fluid_frac(<int> nuc, <int> source, get_errors)
    elif isinstance(nuc, basestring):
        ingest = cpp_data.ingest_df(<char *> nuc, <int> source, get_errors)
        fluid_frac = cpp_data.fluid_frac(<char *> nuc, <int> source, get_errors)
    else:
        raise pyne.nucname.NucTypeError(nuc)

    return ingest, fluid_frac
...
def inhale_df(nuc, source=0, get_errors=False):
    ...
    if isinstance(nuc, int):
        inhale = cpp_data.inhale_df(<int> nuc, <int> source, get_errors)
        lung_model = cpp_data.lung_model(<int> nuc, <int> source, get_errors)
    elif isinstance(nuc, basestring):
        inhale = cpp_data.inhale_df(<char *> nuc, <int> source, get_errors)
        lung_model = cpp_data.lung_model(<char *> nuc, <int> source, get_errors)
    else:
        raise pyne.nucname.NucTypeError(nuc)

    return inhale, lung_model
