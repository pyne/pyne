...
class Isotxs(_BinaryReader):
    ...
    def __init__(self, filename):
        ...
        #: List of nuclides in ISOTXS file.
        self.nuclides = []
    ...
    def _read_file_control(self):
        ...
        self.fc['ngroup'] = fc.get_int()[0]

        # Number of isotopes in file
        self.fc['niso'] = fc.get_int()[0]
        ...
        self.fc['maxord'] = fc.get_int()[0]

        # File-wide fission spectrum flag
        self.fc['ichidst'] = fc.get_int()[0]
    ...
    def _read_nuclide_data(self, nuc):
        ...
        This record contains non-mg data like atomic mass, temperature, and
        some flags.
        """
        ...
        # absolute nuclide label
        nuc.libParams['nuclide'] = r.get_string(8)[0].strip()

        # library name (ENDFV, etc. )
        nuc.libParams['libName'] = r.get_string(8)[0]
        nuc.libParams['isoIdent'] = r.get_string(8)[0]

        # gram atomic mass
        nuc.libParams['amass'] = r.get_float()[0]
        ...
        # nuclide temperature (K)
        nuc.libParams['temp'] = r.get_float()[0]
        ...
        nuc.libParams['classif'] = r.get_int()[0]

        # fission spectrum flag
        nuc.libParams['chiFlag'] = r.get_int()[0]

        # (n,f) cross section flag
        nuc.libParams['fisFlag'] = r.get_int()[0]

        # (n,alpha) cross section flag
        nuc.libParams['nalph'] = r.get_int()[0]

        # (n,p) cross section flag
        nuc.libParams['np'] = r.get_int()[0]

        # (n,2n) cross section flag
        nuc.libParams['n2n'] = r.get_int()[0]

        # (n,d) cross section flag
        nuc.libParams['nd'] = r.get_int()[0]

        # (n,t) cross section flag
        nuc.libParams['nt'] = r.get_int()[0]

        # number of moments of total xs
        nuc.libParams['ltot'] = r.get_int()[0]
        ...
        if nuc.libParams['chiFlag'] == 1:
            ...
            if nuc.libParams['fisFlag'] > 0:
                ...
                assert self.fc['ichidst'] == 1, \
                    "Fissile nuclide {0:s} in library but no individual or" \
                    " global chi!".format(nuc)
    ...
    def _read_nuclide_scatter(self, nuc, block, subBlock):
        ...
        for j in range(jl, ju+1):
            ...
            # convert to groups starting at 0
            g = j - 1
        ...
        for order in range(lordn):
            ...
            for j in range(jl, ju+1):
                ...
                for k in fromgroups:
                    fromg = k-1
                    nuc.micros['scat', block, g, fromg, order] = \
                        r.get_float()[0]
...
class Dlayxs(_BinaryReader):
    """A Dlayxs object represents the data stored in a CCCC-format DLAYXS file.
    This file contains delayed neutron precursor yields, emission spectra, and
    decay constants reduced to multigroup form. Typically, the data in a
...
class Matxs(_BinaryReader):
    """A Matxs object represents data stored in a MATXS file. This file
    contains generalized cross-sections.
...
class _Nuclide(object):
    ...
    Touran had his own Nuclide class so this one is provided to supply the
