...
from sympy import Symbol, pprint, latex, diff, count_ops, simplify, cse, Eq, \
    Q, log, logcombine, Abs, exp, sqrt, series, separate, powsimp, collect, \
    expand, Abs
...
def cgen_ncomp(ncomp=3, nporder=2, aggstat=False, debug=False):
    """Generates a C function for ncomp (int) number of components.  The jth
    key component is always in the first position and the kth key component is
    always in the second.  The number of enrichment stages (NP) is calculated
    via a taylor series approximation.  The order of this approximation may be
    set with nporder.  Only values of 1 or 2 are allowed. The aggstat argument
    determines whether the status messages should be aggreated and printed at
    ...
    # NP : Enrichment Stages
    NP = Symbol('NP', positive=True, real=True)
    # NT : De-enrichment Stages
    NT = Symbol('NT', positive=True, real=True)
    # NP0 : Enrichment Stages Initial Guess
    NP0 = Symbol('NP0', positive=True, real=True)
    # NT0 : De-enrichment Stages Initial Guess
    NT0 = Symbol('NT0', positive=True, real=True)
    # NP1 : Enrichment Stages Computed Value
    NP1 = Symbol('NP1', positive=True, real=True)
    # NT1 : De-enrichment Stages Computed Value
    NT1 = Symbol('NT1', positive=True, real=True)
    ...
    stat = _aggstatus(stat,
                      "  order-{0} NP approximation".format(nporder), aggstat)
...
def cgen_source_file(hfname="temp", max_ncomp=40, debug=False):
    """ Generates a valid C/C++ source file for multicomponent enrichment
    cascades.
    """
...
def cgen_file(filename="temp", header_filename=None, lang='C++', max_ncomp=40,
              debug=False):
    """Generate C/C++ header and source file to compute multicoponent
    enrichment cascades for a number of components between 3 and max_ncomp. The
    filename argument should not end in extension ('.h', '.c', or '.cpp') as it
