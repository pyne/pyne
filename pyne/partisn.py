...
def write_partisn_input(mesh, hdf5, ngroup, **kwargs):
    ...
    commented-out. These suggested cards are the additional cards required for
    a minimum working PARTISN input file:
    ...
    mesh : PyNE mesh
        ...
        must correspond to the desired PARTISN coarse mesh intervals. By
        default one fine mesh inverval per coarse mesh will be used. This can
        be changed with the fine_per_coarse parameter. The sum of all fine mesh
        intervals in the problem must be greater than or equal to 7. Mesh can
        be 1-D (Nx1x1 mesh), 2-D (NxMx1 mesh), or 3-D (NxMxP mesh). Only
        Cartesian meshes are currently supported.
    hdf5 : string
    ...
    if 'names_dict' in kwargs:
        ...
        mat_lib, unique_names = _get_material_lib(hdf5,
                                                  data_hdf5path,
                                                  nuc_hdf5path,
                                                  nuc_names=nuc_names)
    ...
    block02['zones'], block04['assign'] = _get_zones(mesh,
                                                     hdf5,
                                                     bounds,
                                                     num_rays,
                                                     grid,
    ...
    _write_input(title, block01, block02, block03, block04, block05, cards,
                 input_file)
        ...
        for nucid in mat_lib[mat]:
            if names_tf:
                if nucid in nuc_names:
                    ...
                    warn("Nucid {0} does not exist in the provided nuc_names"
                         " dictionary.".format(nucid))
                    mat_xs_names[mat]["{0}".format(nucid)] = \
                        mat_lib[mat][nucid]
            else:
...
def _get_zones(mesh, hdf5, bounds, num_rays, grid, dg, unique_names):
    ...
    # Remove vacuum or graveyard from material definition if not vol_frac of
    # 1.0
        ...
        if zones[z]['mat'] in skip_array and zones[z]['vol_frac'] == [1.0]:
            ...
            for i, mat in enumerate(zones[z]['mat']):
                if mat not in skip_list:
                    ...
                    zones_compressed[z]['vol_frac'] \
                        .append(zones[z]['vol_frac'][i])
    ...
    for i, vals in zones_compressed.iteritems():
        ...
        if first or not match:
            # Check that the material is not 100% void (assign zone 0
            # otherwise)
    ...
    for z in zones_mats:
        ...
        for i, mat in enumerate(zones_mats[z]['mat']):
            if mat not in skip_list:
                ...
                zones_novoid[z]['vol_frac'] \
                    .append(zones_mats[z]['vol_frac'][i])
...
def _check_fine_mesh_total(block01):
    ...
    if total < 7:
        warn("Please supply a larger mesh. Number of fine mesh intervals is "
             "less than 7.")


def _write_input(title, block01, block02, block03, block04, block05, cards,
                 file_name):
    ...
    partisn += "/ Note: NAMES is not all inclusive. Only NAMES that are" \
               " present in\n"
    ...
    missing_3 = set(['lib', 'lng', 'maxord', 'ihm', 'iht', 'ihs', 'ifido',
                     'ititl']) - set(block3_cards)
    if len(missing_3) > 0:
        partisn += "/ Please provide input for the following variables:\n"
    ...
    header += "/\n"
    if default_source:
        header += "/ NOTE: This input includes a default source that is " \
                  "isotropic\n"
        header += "/       in direction and uniform in space and energy.\n"
    if len(missing_1) > 0 or len(missing_3) > 0:
        header += "/ NOTE: The follow commented out cards must be filled " \
                  "in for\n"
...
def isotropic_vol_source(geom, mesh, cells, spectra, intensities, **kwargs):
    ...
    grid : boolean, optional, default = False
        ...
        starting points is used, with dimension sqrt(num_rays) x
        sqrt(num_rays).  In this case, "num_rays" must be a perfect square.
    ...
    dg : record array
        ...
        This array is returned in sorted order with respect to idx and cell,
        with cell changing fastest.
    """
