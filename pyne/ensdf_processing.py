...
class SetupIncompleteError(Exception):
    ...
    def __str__(self):
        """ TBD """
        return "This ensdf analysis module requires additional setup.  " \
            "Run pyne.ensdf_processing.setup_additional_downloads to " \
            "download the additional executables and complete this modules " \
            "instalation. (following executable not yet " \
            "installed/downloaded: " + repr(self.value)
...
def bricc(inputdict_unchecked):
    """
    This function calculates the conversion electron, electron-positron pair
    conversion coefficients, and the E0 electron factors.
    ...
    # check if BriIccHome environment variable has been set (needed by BRICC
    # executable)
    ...
    inp = (inputdict_unchecked['calculation_report']
           if 'calculation_report' in inputdict_unchecked else '') + '\n' \
        + (inputdict_unchecked['G_SG_records']
           if 'G_SG_records' in inputdict_unchecked else '') + '\n' \
        + (inputdict_unchecked['comparison_report']
           if 'comparison_report' in inputdict_unchecked else '') + '\n' \
        + (inputdict_unchecked['conversion_coefficients']
           if 'conversion_coefficients'
           in inputdict_unchecked else '') + '\n' \
        + (inputdict_unchecked['calculate_conversion_coefficients']
           if 'calculate_conversion_coefficients'
           in inputdict_unchecked else '') + '\n' \
        + (inputdict_unchecked['lowest_cc_value']
           if 'lowest_cc_value' in inputdict_unchecked else '') + '\n' \
        + (inputdict_unchecked['assumed_mr_value']
           if 'assumed_mr_value' in inputdict_unchecked else '') + '\n\r\n'
...
def delta(inputdict_unchecked):
    ...
    inputdict_unchecked : dictionary
        dictionary that must have the following key-pair values:
            ...
            output_file : string, path to file for output write (doesn't have
                          to exist).
...
def gtol(inputdict_unchecked):
    """
    GTOL uses gamma-ray energies to derive a set of least-squares adjusted
    level energies.

    The net feeding at each level is calculated from the input gamma
    intensities and conversion coefficients. (GTOL readme)
    ...
    inputdict_unchecked : dictionary
        dictionary that must have the following key-pair values:
            ...
            new_ensdf_file_with_results : boolean, if true then a new ensdf
                                          file with results will be created.
            output_file : string, desired gtol output file path.
            supress_gamma_comparison : boolean, if true the gamma comparison
                                       will be suppressed.
    ...
    -------
    rtn : dictionary
        Everything in input dictionary is returned if GTOL completes
        successfully.
    """
    inputdict = {}
    input_file = inputdict_unchecked['input_file']
...
def radd(inputdict_unchecked):
    ...
    that radius parameter ( r 0 Z , N ) for odd-Z and odd-N nuclides lies
    midway between the radius parameters of adjacent even-even neighbors
...
def radlist(inputdict_unchecked):
    ...
    inputdict_unchecked : dictionary
        dictionary that must have the following key-pair values:
            ...
            output_ensdf_like_file : string, 'Y' if output ensdf like file is
                                     desired, else 'N'.
            output_file_for_nudat : string, 'Y' if output file for nudat is
                                    desired, else 'N'.
            output_mird_listing : string, 'Y' if output mird listing is
                                  desired, else 'N'.
    ...
    if 'input_masses_data_table' in inputdict_unchecked:
        input_masses_data_table = \
            inputdict_unchecked['input_masses_data_table']
    else:
        input_masses_data_table = ''
...
def ruler(inputdict_unchecked):
    ...
    ruler_output = subprocess.Popen([exe_path],
                                    stdout=subprocess.PIPE,
                                    stdin=subprocess.PIPE)
    inp = input_file + '\n' + output_report_file + '\n' \
        + mode_of_operation + '\n' + assumed_dcc_theory
