"""This module accesses the AHOTN, DGFEM, and SCT-STEP flavored neutron
transport solvers.
"""
...
def _error_to_string(error_code):
    ...
    err_dictionary = {
        ...
        1015: "ERROR: Illegal value in material map. Must be in "
              "[1, #materials].",
        ...
        1028: "ERROR: Illegal value for moments to converge, iall. Must be in"
              " [0, lambda].",
        1029: "ERROR: Illegal value for solution check flag, iall. iall is 0"
              " (skip check) or positive integer",
        1030: "ERROR: Illegal value for solution check tolerance, tchk. Must"
              " be positive.",
        1031: "ERROR: Illegal value for direction cosine. Must be entered "
              "positive, less than 1.",
        1032: "ERROR: Illegal value for weight. Must be entered positive, "
              "less than 0.125.",
        1033: "ERROR: a discrete ordinate has length not in range "
              "0.99<1.00<1.01 based on mu, eta, xi values.",
        1034: "ERROR: Illegal value for max moment to print, momp. Must be "
              "between 0 and lambda.",
        1035: "ERROR: Illegal value for flag for moment summing. "
              "Must be 0 for off or 1 for on.",
        1036: "ERROR: Illegal value for flag for moment sum at "
              "non-center point. Must be 0/1=off/on.",
        1037: "ERROR: Illegal value for flag for printing average flux "
              "of the four quadrants. Must be 0/1 = off/on."
    }
...
def _dict_complete(inputdict):
    ...
    try:
        formatted_dict['angular_quadrature_order'] = \
            inputdict['angular_quadrature_order']
    except:
    ...
    try:
        formatted_dict['angular_quadrature_type'] = \
            inputdict['angular_quadrature_type']
    except:
    ...
    assert 'x_boundry_conditions' \
        in inputdict, 'x_boundry_conditions not in dict'
    formatted_dict['x_boundry_conditions'] = inputdict['x_boundry_conditions']
    assert 'y_boundry_conditions' \
        in inputdict, 'y_boundry_conditions not in dict'
    formatted_dict['y_boundry_conditions'] = inputdict['y_boundry_conditions']
    assert 'z_boundry_conditions' \
        in inputdict, 'z_boundry_conditions not in dict'
    ...
    try:
        formatted_dict['convergence_criterion'] = \
            inputdict['convergence_criterion']
    except:
        formatted_dict['convergence_criterion'] = 1e-12
        warn(warning_msg + " convergence_criterion value of {0:0.1E}"
             .format(formatted_dict['convergence_criterion']))

    try:
        formatted_dict['max_iterations'] = inputdict['max_iterations']
    except:
        formatted_dict['max_iterations'] = 6000
        warn(warning_msg + " max_iterations value of {0:d}"
             .format(formatted_dict['max_iterations']))

    try:
        formatted_dict['moments_converged'] = inputdict['moments_converged']
    except:
        formatted_dict['moments_converged'] = 0
        warn(warning_msg + " moments_converged value of {0:d}"
             .format(formatted_dict['moments_converged']))

    try:
        formatted_dict['converge_tolerence'] = inputdict['converge_tolerence']
    except:
        formatted_dict['converge_tolerence'] = 1e-10
        warn(warning_msg + " converge_tolerance value of {0:0.1E}"
             .format(formatted_dict['converge_tolerance']))
