...
std::string pyne::Tally::mcnp(int tally_index, std::string mcnp_version,
  ...
  if (entity_type.find("Surface") != std::string::npos) {
    ...
    if (entity_geometry.find("Cartesian") != std::string::npos) {
      ...
      output << "CYL ";
      if (!is_zero(axl)) {
        sup_var << indent_block << "AXL=" << mcnp_to_string(axl);
      }
      if (!is_zero(vec)) {
        sup_var << "\n" << indent_block;
        sup_var << "VEC=" << mcnp_to_string(vec);
      }
    }

    output << " ORIGIN=" << mcnp_to_string(origin) << "\n";
    ...
    for (int j = 0; j < 3; j++) {
      output << indent_block;
      output << dir_name[j] << "MESH=" << mcnp_to_string(meshes[j]);
      if (bins[j].size() > 0) {
        output << " " << dir_name[j] << "INTS=" << mcnp_to_string(bins[j]);
      }
      output << "\n";
    }
    ...
    if (e_bounds.size() > 0) {
      ...
      output << mcnp_to_string(e_bounds);
    }
    output << "\n";
    if (e_bins.size() > 0) {
      output << indent_block << "EINTS=";
      output << mcnp_to_string(e_bins);
    }
    if (out.size() > 0) {
