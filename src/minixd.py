            ...
            if ";" in line:
                ...
                if name is not None:
                    if dtype in supported_types:
                        ...
                        pass
                        #objects['unmatched'][name] = dtype
                        #objects['docs'][name] = docs.rstrip('\n')
...
def build_pxd(header, altname= None):
    ...
    with open('cpp_' + pxdname + ".pxd", 'w') as f:
        f.write("from libcpp.vector cimport vector, pair\n")
        f.write("\n")
