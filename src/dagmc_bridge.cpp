...
// not callable by the cython layer
DagMC* dagmc_instance() {
  //  return &DAG;
  return &DAG;
  //if (!DAG) 
  //  DAG = new DagMC();
...
EntityHandle handle_from_id(int dimension, int id) {
    return dagmc_instance()->entity_by_id(dimension, id);
}

int id_from_handle(EntityHandle eh) {
    return dagmc_instance()->get_entity_id(eh);
}

ErrorCode dag_load(const char* filename){
    ErrorCode err;

    err = dagmc_instance()->load_file(filename);
    CHECKERR(err);
    err = dagmc_instance()->init_OBBTree();
    CHECKERR(err);
    ...
    err = dagmc_instance()->parse_properties(metadata_keys, metadata_synonyms);
    CHECKERR(err);

    int num_surfs = dagmc_instance()->num_entities(2);
    surfList.reserve(num_surfs);
    for(int i = 1; i <= num_surfs; ++i) {
        surfList.push_back(dagmc_instance()->id_by_index(2, i));
    }

    int num_vols = dagmc_instance()->num_entities(3);
    volList.reserve(num_vols);
    for(int i = 1; i <= num_vols; ++i) {
        volList.push_back(dagmc_instance()->id_by_index(3, i));
    }
...
ErrorCode dag_ray_fire(EntityHandle vol, vec3 ray_start, vec3 ray_dir, 
    ...
    err = dagmc_instance()->ray_fire(vol, ray_start, ray_dir, *next_surf_ent, *next_surf_dist, 
...
ErrorCode dag_ray_follow(EntityHandle firstvol, vec3 ray_start, vec3 ray_dir,
    ...
    while(vol) {
        err = dagmc_instance()->ray_fire(vol, ray_point.array(), ray_dir, 
        ...
        if(next_surf) {
            ...
            err = dagmc_instance()->next_vol(next_surf, vol, vol);
            CHECKERR(err);
...
ErrorCode dag_pt_in_vol(EntityHandle vol, vec3 pt, int* result, vec3 dir, const void* history) {
    ...
    err = dagmc_instance()->point_in_volume(vol, pt, *result, dir, static_cast<const DagMC::RayHistory*>(history));
...
ErrorCode dag_next_vol(EntityHandle surface, EntityHandle volume, EntityHandle* next_vol) {
    ...
    err = dagmc_instance()->next_vol(surface, volume, *next_vol);

    return err;
}

int vol_is_graveyard(EntityHandle vol) {
    return dagmc_instance()->has_prop(vol, "graveyard");
}
...
int vol_is_implicit_complement(EntityHandle vol){
    return dagmc_instance()->is_implicit_complement(vol);
}
...
ErrorCode get_volume_metadata(EntityHandle vol, int* material, double* density, double* importance) {
    ...
    std::string str;
    err = dagmc_instance()->prop_value(vol, "mat", str);
    ...
    err = dagmc_instance()->prop_value(vol, "rho", str);
    if(err == moab::MB_SUCCESS) {
    ...
    err = dagmc_instance()->prop_value(vol, "imp", str);
    if(err == moab::MB_SUCCESS) {
...
ErrorCode get_volume_boundary(EntityHandle vol, vec3 minPt, vec3 maxPt) {
    return dagmc_instance()->getobb(vol, minPt, maxPt);
}
