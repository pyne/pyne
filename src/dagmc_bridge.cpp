...
float dag_version(void) { return DagMC::version(); }

unsigned dag_rev_version(void) { return DagMC::interface_revision(); }

int dag_ent_handle_size(void) { return sizeof(EntityHandle); }
...
int id_from_handle(EntityHandle eh) { return DAG->get_entity_id(eh); }
...
void* dag_alloc_ray_history(void) { return new DagMC::RayHistory(); }
...
ErrorCode get_volume_metadata(EntityHandle vol, int* material, double* density,
  ...
  // the defaults from DagMC's old get_volume_metadata: mat = 0, rho = 0, imp =
  // 1
  int mat_id = 0;
  double rho = 0, imp = 1;
