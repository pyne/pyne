module dgfem_complete
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
! This module contains subroutines and variables pertinent
! for setting up the mass, stiffness and face matrices for
! a given order Lambda for the DGFEM complete family. It 
! also contains the dgfem_cmplt_kernel which solves the 
! resulting set of equations for a single cell. The matrices are
! treated as two-dimensional dense arrays. 
!
! Complete family: P_i(x)*P_j(y)*P_k(z) for i=0,..,Lambda ; j=0,..,Lambda-i ;
!                                           k=0,..,Lambda-i-j
!
! ** 1/6*(Lambda+3)*(Lambda+2)*(Lambda+1)  DoFs per cell
...
!                      i=iz+1-1/2*iy*(-3+2*ix+iy-2*Lambda)+
!                        1/6*ix*(11+ix**2*-3*ix*(2+Lambda)+3*Lambda*(4+Lambda))
! 
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!
implicit none
...
!                               x,y,z axes, respectively 
!      tfsx,tfsy,tfsz:          tface<x,y,z>O-tstiff<x,y,z>       
!
